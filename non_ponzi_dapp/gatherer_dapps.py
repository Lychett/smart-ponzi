...
# apertura dei file
non_ponzi_dapp_addr = "non_ponzi_dapps_addr.csv"              # indirizzi smart contract
non_ponzi_dapp_result = open("non_ponzi_dapps_result.csv", "w+")   # conterra' le features degli smart contract
writer = csv.writer(non_ponzi_dapp_result, lineterminator = '\n') # writer per scrivere sul file
...
with open(non_ponzi_dapp_addr, 'r', encoding = 'utf-8-sig') as f: 
    reader = csv.reader(f, delimiter = ',', quotechar = '|') # reader: oggetto che itera le linee del file csv 
    for row in reader:
        addr = '0x' + row[0] # si legge per righe, il primo elemento di ogni riga viene inserito in addr
...
for addr in non_ponzi: # ciclo su tutti gli indirizzi presenti nella lista
    ...
    for t in result_normal: # transazioni in ingresso nel contratto
        if (t['isError'] == '0'): # assenza di errori nella transazione
            ...
            eth_val = round(Decimal(t['value'])/Decimal('1000000000000000000'), 6)
            if (eth_val > 0): # se il valore della transazione e' > 0 allora...
                if t['from'] in dict_addr_tx_in: # se l'indirizzo e' gia' presente, cioe' il mittente della transazione aveva gia' pagato il contratto
                    ...
                    dict_addr_eth_in[t['from']] += eth_val # incremento il valore associato alla chiave, che indica quanto eth ha inviato
                else:
                    dict_addr_tx_in[t['from']] = 1 # se non era ancora presente allora e' la prima volta che invia soldi al contratto
                    dict_addr_eth_in[t['from']] = eth_val
    ...
    internal_tx_url = "https://api.etherscan.io/api?module=account&action=txlistinternal&address=" + addr + \
                       "&startblock=0&endblock=9999999&page=1&offset=10000&sort=asc&apikey=Q9C796N54G4G4S6JP91ERNQTM9VWC463BP"
    ...
    for t in result_internal:
        ...
        obj_datetime = datetime.fromtimestamp(int(t['timeStamp']))
        eth_val = round(Decimal(t['value'])/Decimal('1000000000000000000'), 6)
        if (t['from'] == addr):  # internal transaction from contract to t['to'] aka addr, flusso di ETH uscente            
            ...
            if (eth_val > 0):
                if t['to'] in dict_addr_tx_out:
                    ...
                    dict_addr_eth_out[t['to']] += eth_val
                else:
    ...
    string_fisrt_date = txs_list_in[0] # Return a string representing the date in fmt y-m-d, questa la prendo da txs_list_in perche' la prima proviene sempre da lista di tx in ingresso
    date_zero = datetime.strptime(string_fisrt_date, '%Y-%m-%d') # ottengo adesso un oggetto date che indica la data di inizio
    date_last_in_tx = datetime.strptime(txs_list_in[-1], '%Y-%m-%d') # questa ce' per forza perche' una transazione entrante serve per creare il contratto
    ...
    balance = eth_in - eth_out                  # bilancio del contratto
