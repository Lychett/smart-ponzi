...
def variance(dict_of_values, mean_of_values): # varianza, per il calcolo della deviazione standard
...
# apertura dei file
ponzi_addr = "ponzi_addresses.csv"              # indirizzi smart contract
bytecodes_result = open("bytecodes.txt", "w+") # conterra' il bytecode degli smart ponzi
ponzi_result = open("ponzi_result.csv", "w+")   # conterra' le features degli smart contract
writer = csv.writer(ponzi_result, lineterminator = '\n') # writer per scrivere sul file
...
ponzi = [] # lista che mantiene gli indirizzi degli smart contract

with open(ponzi_addr, 'r', encoding = 'utf-8-sig') as f: 
...
for addr in ponzi: # ciclo su tutti gli indirizzi presenti nella lista ponzi
